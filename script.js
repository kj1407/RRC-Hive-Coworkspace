        ...
        window.addEventListener('scroll', () => {
            ...
            if(midLayer) midLayer.style.transform = `translateY(${scrollY * 0.4}px)`;
            if(frontLayer) frontLayer.style.transform = `translateY(${scrollY * 0.1}px)`;
        });
...
document.querySelectorAll('.reveal-text-lr').forEach(el => textObserver.observe(el));
    ...
    function confirmBooking() {
        ...
        // Output Confirmation
        console.log("Booking Details:", { name, phone, email, date, start, end, room });
    ...
    // Auto-close offcanvas when any link inside it is clicked
        document.querySelectorAll('.offcanvas-body .nav-link').forEach(link => {
            link.addEventListener('click', () => {
                ...
                const box = bootstrap.Offcanvas.getInstance(offcanvasElement);
                if (box) {
                    box.hide();
                }
            });
        ...
        function confirmBooking() {
            ...
            - Email: ${email}`;
                ...
                const whatsappUrl = `https://wa.me/918903169940?text=${encodeURIComponent(message)}`;
